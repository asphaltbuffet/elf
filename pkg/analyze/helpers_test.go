...
func makeBenchmarkData(year int, days ...int) []*exercise.BenchmarkData {
	out := make([]*exercise.BenchmarkData, 0, len(days))

	for _, d := range days {
		out = append(out, &exercise.BenchmarkData{
			Year: year,
			Day:  d,
			Implementations: []*exercise.ImplementationData{
				{
					Name: "Golang",
					PartOne: &exercise.PartData{
						Mean: 0.001 * float64(d),
						Min:  0.0005 * float64(d),
						Max:  0.002 * float64(d),
						Data: []float64{0.001, 0.0012, 0.0008},
					},
					PartTwo: &exercise.PartData{
...
func makeBenchmarkDataNilPartTwo(year int, days ...int) []*exercise.BenchmarkData {

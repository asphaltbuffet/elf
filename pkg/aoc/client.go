...
func registerExercise(year int, day int, path string, name string) error {
	...
	exercises[year][day] = &exercise.Exercise{
		Name: utilities.CamelToTitle(name),
		Day:  day,
		Dir:  path,
	}
...
func (ac *AOCClient) GetInput(year int, day int) (string, error) {
	...
	// Load exercise input
	input, err := os.ReadFile(filepath.Join(e.Dir, i.InputFile))
	if err != nil {
...
func (ac *AOCClient) ImplementationDirs(year int, day int) ([]string, error) {
	if err := isValidExercise(year, day); err != nil {
		return nil, fmt.Errorf("getting %d-%d exercise: %w", year, day, err)
	}

	base := exercises[year][day].Dir
	dirs := make([]string, 0, len(ac.Runners))
	...
	for r := range ac.Runners {
		...
		if _, err := fs.Stat(path); err == nil {
			...
			fmt.Printf("%s implementation not found: %v\n", path, err)
		}
	}
...
func (ac *AOCClient) MissingDays() (map[int]([]int), error) {
	return nil, fmt.Errorf("not implemented")
}

func (ac *AOCClient) MissingImplementations() (map[int]map[int]string, error) {
	return nil, fmt.Errorf("not implemented")
}
